...
import { buildAdLayoutCandidates, type AdLayoutInput } from "./ad-layout-recipes.ts";

export { buildAdLayoutCandidates, type AdLayoutCandidate, type AdLayoutCandidateId, type AdLayoutInput, type AdLayoutPurpose } from "./ad-layout-recipes.ts";
...
export async function buildAdLayoutLayers(input: AdLayoutInput): Promise<LayerData[]> {
  return buildAdLayoutCandidates(input)[0]?.layers ?? [];
}
